...
if ( ! class_exists( 'Cryptobox_Settings' ) ) {

	/**
	 * Handles plugin settings and user profile meta fields
	 */
	class Cryptobox_Settings extends Cryptobox_Module {
		protected $settings;
		 ...
		 * @param array  $value This will be merged with Cryptobox_Settings->settings, so it should mimic the structure of the Cryptobox_Settings::$default_settings. It only needs the contain the values that will change, though. See Cryptobox_Plugin->upgrade() for an example.
		 */
		public function __set( $variable, $value ) {
			// Note: Cryptobox_Module::__set() is automatically called before this
			...
			update_option( 'cryptobox_settings', $this->settings );
		}
		...
		protected static function get_settings() {
			$settings = shortcode_atts(
				...
				get_option( 'cryptobox_settings', array() )
			);
		...
		public static function add_plugin_action_links( $links ) {
			array_unshift( $links, '<a href="options-general.php?page=' . 'cryptobox_settings">Settings</a>' );
		...
		public static function markup_settings_page() {
			if ( current_user_can( self::REQUIRED_CAPABILITY ) ) {
				echo self::render_template( 'cryptobox-settings/page-settings.php' );
			} else {
		...
		public function register_settings() {
			...
			add_settings_field(
				'cryptobox_default_currency',
				'Default currency',
				array( $this, 'markup_fields' ),
				'cryptobox_settings',
				'cryptobox_section-general',
				array( 'label_for' => 'cryptobox_default_currency' )
			);

			add_settings_field(
				'cryptobox_default_price',
				'Default price',
				array( $this, 'markup_fields' ),
				'cryptobox_settings',
				'cryptobox_section-general',
				array( 'label_for' => 'cryptobox_default_price' )
			);
			...
			add_settings_field(
				'cryptobox_field-example2',
				'Example Field 2',
				array( $this, 'markup_fields' ),
				'cryptobox_settings',
				'cryptobox_section-advanced',
				array( 'label_for' => 'cryptobox_field-example2' )
			);
		...
		public static function markup_section_headers( $section ) {
			echo self::render_template( 'cryptobox-settings/page-settings-section-headers.php', array( 'section' => $section ), 'always' );
		}
		...
		public function markup_fields( $field ) {
			switch ( $field['label_for'] ) {
				case 'cryptobox_field-example1':
					// Do any extra processing here
					break;
			}

			echo self::render_template( 'cryptobox-settings/page-settings-fields.php', array( 'settings' => $this->settings, 'field' => $field ), 'always' );
		}
		...
		public function validate_settings( $new_settings ) {
			...
			if ( ! is_string( $new_settings['db-version'] ) ) {
				$new_settings['db-version'] = Cryptobox_Plugin::VERSION;
			}
		...
		public static function add_user_fields( $user ) {
			echo self::render_template( 'cryptobox-settings/user-fields.php', array( 'user' => $user ) );
		}
		...
		public static function save_user_fields( $user_id ) {
			...
			update_user_meta( $user_id, 'cryptobox_user-example-field1', $user_fields[ 'cryptobox_user-example-field1' ] );
			update_user_meta( $user_id, 'cryptobox_user-example-field2', $user_fields[ 'cryptobox_user-example-field2' ] );
		}
		...
		public static function validate_user_fields( $user_id, $user_fields ) {
			if ( $user_fields[ 'cryptobox_user-example-field1' ] == false ) {
				$user_fields[ 'cryptobox_user-example-field1' ] = true;
				add_notice( 'Example Field 1 should be true', 'error' );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				$current_field2 = get_user_meta( $user_id, 'cryptobox_user-example-field2', true );

				if ( $current_field2 != $user_fields[ 'cryptobox_user-example-field2' ] ) {
					$user_fields[ 'cryptobox_user-example-field2' ] = $current_field2;
