...
if ( ! class_exists( 'Cryptobox_CPT_Example' ) ) {

	/**
	 * Creates a custom post type and associated taxonomies
	 */
	class Cryptobox_CPT_Example extends Cryptobox_Module implements Cryptobox_Custom_Post_Type {
		protected static $readable_properties  = array();
		protected static $writeable_properties = array();

		const POST_TYPE_NAME = 'WPPS Custom Post Type';
		const POST_TYPE_SLUG = 'cryptobox-cpt';
		const TAG_NAME       = 'WPPS Custom Taxonomy';
		const TAG_SLUG       = 'cryptobox-custom-tax';
		...
		protected static function get_post_type_params() {
			...
			return apply_filters( 'cryptobox_post-type-params', $post_type_params );
		}
		...
		protected static function get_tag_taxonomy_params() {
			...
			return apply_filters( 'cryptobox_tag-taxonomy-params', $tag_taxonomy_params );
		}
		...
		public static function markup_meta_boxes( $post, $box ) {
			...
			switch ( $box['id'] ) {
				case 'cryptobox_example-box':
					$variables['exampleBoxField'] = get_post_meta( $post->ID, 'cryptobox_example-box-field', true );
					$view                         = 'cryptobox-cpt-example/metabox-example-box.php';
					break;

				/*
				case 'cryptobox_some-other-box':
					$variables['someOtherField'] = get_post_meta( $post->ID, 'cryptobox_some-other-field', true );
				 	$view                        = 'cryptobox-cpt-example/metabox-another-box.php';
					break;
				*/
		...
		public static function is_protected_meta( $protected, $meta_key, $meta_type ) {
			switch( $meta_key ) {
				case 'cryptobox_example-box':
				case 'cryptobox_example-box2':
					$protected = true;
					break;

				case 'cryptobox_some-other-box':
				case 'cryptobox_some-other-box2':
					$protected = false;
					break;
			}
		...
		protected static function save_custom_fields( $post_id, $new_values ) {
			if ( isset( $new_values[ 'cryptobox_example-box-field' ] ) ) {
				if ( false ) { // some business logic check
					update_post_meta( $post_id, 'cryptobox_example-box-field', $new_values[ 'cryptobox_example-box-field' ] );
				} else {
		...
		/**
		 * Defines the [cryptobox-cpt-shortcode] shortcode
		 *
		 * @mvc Controller
		 *
		...
		public static function cpt_shortcode_example( $attributes ) {
			$attributes = apply_filters( 'cryptobox_cpt-shortcode-example-attributes', $attributes );
			$attributes = self::validate_cpt_shortcode_example_attributes( $attributes );

			return self::render_template( 'cryptobox-cpt-example/shortcode-cpt-shortcode-example.php', array( 'attributes' => $attributes ) );
		}
		...
		protected static function validate_cpt_shortcode_example_attributes( $attributes ) {
			...
			return apply_filters( 'cryptobox_validate-cpt-shortcode-example-attributes', $attributes );
		}
		...
		protected static function get_default_cpt_shortcode_example_attributes() {
			...
			return apply_filters( 'cryptobox_default-cpt-shortcode-example-attributes', $attributes );
		}
